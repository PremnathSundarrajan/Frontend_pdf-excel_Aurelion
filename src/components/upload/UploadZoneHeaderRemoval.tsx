...
import { Upload, FileText, Scissors } from "lucide-react";
...
export function UploadZoneHeaderRemoval({ onUpload, disabled }: UploadZoneProps) {
        ...
        >
            ...
            >
                ...
                <div className="flex flex-col items-center gap-5">
                    <motion.div
                        ...
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-200 ${isDragging ? "bg-orange-500 shadow-medium" : "bg-orange-50/80"
                            }`}
                    >
                        <Scissors className={`w-7 h-7 transition-colors duration-200 ${isDragging ? "text-white" : "text-orange-600"
                            }`} />
                    </motion.div>

                    <div className="text-center">
                        <p className="text-foreground font-medium mb-1.5">
                            {isDragging ? "Drop PDF files here" : "Drop multiple PDFs to process"}
                        </p>
                        <p className="text-sm text-muted-foreground">
                            We'll remove headers, footers, and bundle them into a ZIP.
                        </p>
                    </div>
            ...
            {files.length > 0 && (
                <div className="mt-6 w-full max-w-xl mx-auto">
                    <div className="mb-3 font-medium">Files to process ({files.length})</div>
                    ...
                    <div className="mt-4 flex items-center gap-3">
                        ...
                        >
                            {loading ? 'Processing...' : `Process ${files.length} PDF${files.length > 1 ? 's' : ''} & Download ZIP`}
                        </button>
